...
describe("ToDoList", () => {
    ...
    test("get list of incomplete todo items", () => {
        ...
        const dueDate1 = new Date("Feb 01 2024");
        const item1 = new ToDoItem(title1, description1, dueDate1);
        toDoList.addToDoItem(item1);

        const title2 = "Dog";
        const description2 = "Feed Dog";
        const dueDate2 = new Date("Feb 18 2024");
        const item2 = new ToDoItem(title2, description2, dueDate2);
        ...
        const dueDate3 = new Date("March 01 2024");
        const item3 = new ToDoItem(title3, description3, dueDate3);
        toDoList.addToDoItem(item3);

        expect(toDoList.getIncompleteItems()).toStrictEqual([item1, item3]);
    ...
    test("sort todo items by title", () => {
        ...
        const thirdItem = new ToDoItem(title1, description1, dueDate1);
        toDoList.addToDoItem(thirdItem);

        const title2 = "Basketball";
        const description2 = "Play basketball";
        const dueDate2 = new Date("May 31 2023");
        const secondItem = new ToDoItem(title2, description2, dueDate2);
        toDoList.addToDoItem(secondItem);

        const title3 = "Archery";
        const description3 = "Do some archery";
        const dueDate3 = new Date("June 01 2023");
        const firstItem = new ToDoItem(title3, description3, dueDate3);
        toDoList.addToDoItem(firstItem);

        expect(toDoList.sortByTitle()).toStrictEqual([firstItem, secondItem, thirdItem]);
    });
});